...
let Login = ({
    ...
    const validationMap = {
        username: () => { 
            setUserFormError((state) => ({
                ...state,
                username: userForm.username.length === 0? 'Username is required': null
            })); 
        },
        password: () => {
            setUserFormError((state) => ({
                ...state,
                password: userForm.password.length === 0? 'Password is required': null
            }));
        }
    };
    const validateForm = () => {
    ...
    const login = () => {
        ...
        if(formValidation === null) {
            userLogin(userForm.username,userForm.password);
        }
    };
